...
const JobCard = ({ job, onApply, onRoleplay, isRecruiter = false, onEdit, onDelete, onRecommend }) => {
  ...
  const handleCardClick = () => {
    navigate(`/jobs/${job.jobId}`, { state: { job, isRecruiter } });
  };
  ...
  return (
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <h3 className="text-white font-bold text-lg leading-tight truncate group-hover:text-blue-400 transition-colors">
            {job.jobTitle}
          </h3>
          <p className="text-slate-400 text-sm mt-0.5 flex items-center gap-1.5">
            <svg xmlns="http://www.w3.org/2000/svg" className="w-3.5 h-3.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {/* Description — truncated on card, full on detail page */}
      <p className="text-slate-400 text-sm leading-relaxed line-clamp-3">{job.jobDescription}</p>
      ...
      <div
        ...
        className="flex flex-wrap gap-2 mt-auto pt-2 border-t border-slate-700"
      >
        {!isRecruiter && (
          <>
            <button
